...
def raw_to_json(data):   
    try:
        ...
        print(f"Data length: {len(data)}, first bytes: {data[:20].hex() if len(data) >= 20 else data.hex()}")
...
def main():
    ...
    try:
        for i, (fuzzer_file, target_file) in enumerate(zip(fuzzer_files, target_files)):
            ...
            # Skip decoding for now - just send raw data
            if args.verbose:
                print(f"TX: raw binary data ({len(request_bytes)} bytes)")
                print(f"First bytes: {request_bytes[:20].hex() if len(request_bytes) >= 20 else request_bytes.hex()}")
            ...
            try:
                if args.verbose:
                    print(f"Decoding target response... {response_data[:20].hex() if len(response_data) >= 20 else response_data.hex()}")
                response_msg = raw_to_json(response_data)
                if args.verbose:
                    print(f"Decoded target response msg: {response_msg}")
                print_message(response_msg, False, args.verbose)
            except Exception as e:
                print(f"Error decoding target response: {e}")
                if args.verbose:
                    print(f"Response data: {response_data[:50].hex() if len(response_data) >= 50 else response_data.hex()}")
                break
